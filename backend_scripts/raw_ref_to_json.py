...
"""
    ...
    def _merge_authors(self, parsed_authors, s2_authors, openalex_authors=None):
        ...
        for i in range(max_length):
            ...
            mapped_name = self.fake_author_mapping.get(raw_name) if raw_name and not name else name
            ...
            author_key = None
            for identifier_type, identifier in [("orcid", orcid), ("s2_id", s2_id), ("raw_name", raw_name), ("name", candidate["name"])]:
                if identifier and (identifier_type, identifier) in self.author_map:
                    author_key = self.author_map[(identifier_type, identifier)]["key"]
                    existing_author = self.author_map[(identifier_type, identifier)]["author"]
                    existing_author["name"] = candidate["name"] or existing_author["name"]
                    existing_author["s2_id"] = candidate["s2_id"] or existing_author["s2_id"]
                    existing_author["orcid"] = candidate["orcid"] or existing_author["orcid"]
                    existing_author["raw_name"] = candidate["raw_name"] or existing_author["raw_name"]
                    break
            ...
            if not author_key:
                ...
                if orcid:
                    self.author_map[("orcid", orcid)] = {"author": candidate, "key": author_key}
                if s2_id:
                    self.author_map[("s2_id", s2_id)] = {"author": candidate, "key": author_key}
                if raw_name:
                    self.author_map[("raw_name", raw_name)] = {"author": candidate, "key": author_key}
                if candidate["name"]:
                    self.author_map[("name", candidate["name"])] = {"author": candidate, "key": author_key}
    ...
    def enrich_paper_data(self, parsed_data):
        ...
        for i, paper in enumerate(parsed_data, 1):
            ...
            if skip_apis:
                print(f"DEBUG: Paper {i} with title '{title}' detected as artificial. Skipping APIs.")
                enriched_paper = {
                    "title": paper.get("title"),
            ...
            if need_openalex:
                print(f"DEBUG: ORCID missing or Semantic Scholar data not available for paper {i}, attempting OpenAlex")
    ...
    def _format_authors(self, authors):
        ...
        for raw_name in authors:
            ...
            if ("raw_name", raw_name) in self.author_map:
                ...
                author_key = f"author-{self.author_counter}"
                self.author_counter += 1
                author_dict = {"name": mapped_name, "s2_id": None, "orcid": None, "raw_name": raw_name}
                self.author_map[("raw_name", raw_name)] = {"author": author_dict, "key": author_key}
                self.author_map[("name", mapped_name)] = {"author": author_dict, "key": author_key}
    ...
    def _merge_data_openalex(self, parsed, openalex):
        ...
        merged = {
            "title": title,
            "authors": self._merge_authors(parsed.get('authors', []), [], openalex_authors),
    ...
    def assign_entity_keys(self, enriched):
        entity_keys = {"authors": {v["key"]: v["author"] for v in self.author_map.values()},
                       "citations": {}, "venues": {}}
        venue_key_map = {}
        venue_counter = 1
        citation_counter = 1
