...
class PaperProcessor:
    ...
    def __init__(self):
        ...
        self.fake_author_mapping = {
            "H. Solo": "Han Solo", "L. Skywalker": "Luke Skywalker",
            "O.-W. Kenobi": "Obi-Wan Kenobi", "D. Smeesters": "Dirk Smeesters",
            "J. Liu": "Jia Liu",  "J. Hutt": "Jabba Hutt",   "L. Organa": "Leia Organa",
            "Q.-G. Jinn": "Qui-Gon Jinn",     "A. Skywalker": "Anakin Skywalker",
            "M. Windu": "Mace Windu",         "Chewbacca": "Chewbacca",
            "D. Vader": "Darth Vader",
        }
...
"""
        try:
            out   = self.gpt_client.chat.completions.create(
                        model="gpt-3.5-turbo",
            ...
            data  = json.loads(out)
            if "authors" in data:
                data["authors"] = [self.fake_author_mapping.get(a, a)
    ...
    def search_paper(self, title):
        try:
            r = requests.get(f"{self.s2_base}/paper/search",
                             params={"query": title, "fields": "paperId", "limit": 1},
                             headers={"x-api-key": self.s2_api_key}, timeout=15)
            r.raise_for_status()
            d = r.json()
    ...
    def get_s2(self, pid):
        fields = ("title,authors.name,authors.authorId,authors.externalIds,"
                  "venue,year,citationCount,fieldsOfStudy,externalIds,paperId")
        try:
            r = requests.get(f"{self.s2_base}/paper/{pid}",
    ...
    def merge_authors(self, parsed, s2_list, oa_list=None):
        ...
        for i in range(maxlen):
            ...
            cand  = {"name": canon or raw, "s2_id": s2_id,
                     "orcid": orcid, "raw_name": raw}

            key = None
            for t, ident in [("orcid", orcid), ("s2_id", s2_id),
                             ("raw_name", raw), ("name", cand["name"])]:
                if ident and (t, ident) in self.author_map:
                    key = self.author_map[(t, ident)]["key"]; break
            if not key:
                key = f"author-{self.author_counter}"; self.author_counter += 1
                for t, ident in [("orcid", orcid), ("s2_id", s2_id),
                                 ("raw_name", raw), ("name", cand["name"])]:
                    if ident:
    ...
    def merge_oa(self, gpt, oa):
        oa_auth = [{"name": a["author"]["display_name"], "orcid": a["author"]["orcid"]}
        ...
        return {
            ...
            "raw_venue": gpt.get("raw_venue") or \
                         oa.get("host_venue", {}).get("display_name"),
    ...
    def run(self):
        ...
        for refid in flor.loop("refid", sorted(ref_string_by_id.keys())):
            ...
            need_oa = not s2 or any(
                not (a.get("externalIds") or {}).get("ORCID")
                for a in s2.get("authors", [])) if s2 else True
            oa = self.get_oa(gpt["title"]) if need_oa and gpt.get("title") else None

            if s2:
                oa_auth = [{"name": au["author"]["display_name"],
                            "orcid": au["author"]["orcid"]}
                           for au in oa.get("authorships", [])] if oa else None
