...
hop0_citation_ids = {c["id"] for c in citations_data["citations"] if c.get("hop") == 0}
hop1_citation_ids = {c["id"] for c in citations_data["citations"] if c.get("hop") == 1}

author_to_citations = {
    author["id"]: set(author.get("citation", []))
    for author in authors_data["authors"]
}

hop0_authors = {aid for aid, cits in author_to_citations.items() if cits & hop0_citation_ids}
hop1_authors = {aid for aid, cits in author_to_citations.items() if cits & hop1_citation_ids}
hop0_or_hop1_authors = hop0_authors.union(hop1_authors)

# 8. Filter suspicious SCCs with at least one hop-0 OR hop-1 author
hop01_sccs = [scc for scc in suspicious_sccs if any(a in hop0_or_hop1_authors for a in scc)]
...
weights = [suspicious_subgraph[u][v]['weight'] for u, v in suspicious_subgraph.edges()]
nx.draw(
    suspicious_subgraph,
    pos,
...
# 14. Export to JSON: All suspicious SCCs
with open("./outputs/suspicious_sccs.json", "w") as f:
    json.dump({
...
print("✅ Original suspicious SCCs saved to: ./outputs/suspicious_sccs.json")

# 15. Export to JSON: Hop-0 or Hop-1 SCCs
with open("./outputs/suspicious_hop01_sccs.json", "w") as f:
    json.dump({
        "nodes": [{"id": n, "group": node_group_hop01[n]} for n in G.nodes()],
        "links": [{"source": u, "target": v, "value": d["weight"]} for u, v, d in G.edges(data=True)]
    }, f, indent=2)
print("✅ Suspicious SCCs (hop-0 or hop-1) saved to: ./outputs/suspicious_hop01_sccs.json")

# 16. Export detailed SCC info
with open("./outputs/scc_details_with_hop0_or_hop1.json", "w") as f:
    json.dump({
        "original_sccs": [
            {
...
print("✅ Detailed SCC info saved to: ./outputs/scc_details_with_hop0_or_hop1.json")
