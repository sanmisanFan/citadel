...
def detect_suspicious_venues(citations, venues):
    citation_to_venue = {
        c_id: citation["venue"] for c_id, citation in citations.items()
    }
    venue_to_name = {venue["id"]: venue["standardized_name"] for venue in venues}
    ...
    for citation in citations.values():
        ...
        cited_citation_ids = get_citation_keys(citation)  # List of cited papers
    ...
    for scc in suspicious_sccs:
        ...
        if any(ven in hop0_venues for ven in scc) or any(
            ven in hop1_venues for ven in scc
        ):
            hop_sccs.append(scc)
    ...
    export_data_hop = {
        "nodes": [
            {"id": node, "group": node_group_hop[node]} for node in G_venue.nodes()
        ],
        "links": [
            {"source": u, "target": v, "value": data["weight"]}
            for u, v, data in G_venue.edges(data=True)
        ...
        "all_suspicious_sccs": [
            {
                ...
                "edges": [
                    {"source": u, "target": v, "weight": G_venue[u][v]["weight"]}
    ...
    return export_data_suspicious, export_data_hop, scc_details
